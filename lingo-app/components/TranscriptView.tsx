    ...
    return (
      <View style={styles.empty}>
        ...
        <Text style={styles.emptySub}>
          Speak — tap any word or phrase{'\n'}to get an instant explanation
        </Text>
      </View>
    );
  }
            ...
            {seg.tokens.map((tok) =>
              tok.isWord ? (
                <TouchableOpacity
                  key={tok.id}
                  activeOpacity={0.6}
                  onPress={() => onWordPress(tok.text, segText)}
                  style={styles.wordBtn}
                >
                  <Text style={styles.word}>{tok.text}</Text>
                </TouchableOpacity>
              ) : (
                <Text key={tok.id} style={styles.punct}>
                  {tok.text}
                </Text>
              )
            )}
...
const styles = StyleSheet.create({
  ...
  emptyTitle: {
    fontSize: 20, fontFamily: F.bodySemi, color: C.textSub,
    marginBottom: 10, textAlign: 'center',
  },
  emptySub: {
    fontSize: 15, fontFamily: F.body, color: C.textDim,
