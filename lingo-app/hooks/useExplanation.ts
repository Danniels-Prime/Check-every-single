...
export function useExplanation() {
  ...
  const fetch = useCallback(
    async (term: string, context: string, language: Language, apiKey: string) => {
      if (!apiKey.trim()) {
        setState({ phase: 'error', message: 'Add your Claude API key in Settings.' });
        return;
      }
      ...
      try {
        ...
        setState({
          phase:   'error',
          message: e instanceof Error ? e.message : 'Failed to get explanation.',
        });
      }
    },
    []
